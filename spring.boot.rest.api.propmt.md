...
**1. Use @RestController for RESTful Services:**
  - Mark your controllers with `@RestController` for convenience. It's a stereotype annotation that combines `@Controller` and `@ResponseBody`.
 
**2. Use @Service for Business Logic:**
  - Annotate your service classes with `@Service`. This helps in component scanning and indicates that the class holds business logic.
 
**3. Use @Repository for Data Access:**
  - Use `@Repository` for classes that interact with the database. It also provides automatic exception translation.
 
**4. Prefer Constructor Injection for Dependencies:**
  - Use constructor injection (`@Autowired` on constructor) over field injection or setter injection. It promotes immutability and testability.
 
**5. Avoid Excessive @Autowired:**
  - If a class has too many dependencies, it might indicate a violation of the Single Responsibility Principle. Refactor if necessary.
 
**6. Define Separate DTOs for API Communication:**
  - Never expose JPA entities directly through your REST APIs. Use Data Transfer Objects (DTOs) for requests and responses to decouple the API from the persistence layer.
 
**7. Use Bean Validation (JSR 380) for Input Validation:**
  - Apply `@Valid` annotation to DTOs in controller methods and use validation annotations (`@NotNull`, `@Size`, `@Pattern`, etc.) within DTOs.
 
**8. Implement Global Exception Handling:**
  - Use `@ControllerAdvice` or `@RestControllerAdvice` with `@ExceptionHandler` methods to handle exceptions consistently across your application.
 
**9. Log Effectively:**
  - Use a logging framework (e.g., SLF4J with Logback). Configure logging levels appropriately. Avoid logging sensitive information.
 
**10. Use Application Properties for Configuration:**
  - Externalize configuration using `application.properties` or `application.yml`. Use profiles for different environments (e.g., `application-dev.properties`, `application-prod.properties`).
 
**11. Secure Your Endpoints:**
  - Implement Spring Security for authentication and authorization. Use CSRF protection, HTTPS, and proper access control.
 
**12. Implement Caching where Appropriate:**
  - Use Spring's caching abstraction (`@Cacheable`, `@CachePut`, `@CacheEvict`) for frequently accessed, less volatile data to improve performance.
 
**13. Database Transactions:**
  - Use `@Transactional` on service methods that perform database write operations to ensure atomicity. Specify `readOnly=true` for read operations.
 
**14. Use Spring Data JPA for Repositories:**
  - Leverage Spring Data JPA to simplify data access layers. Follow conventions for derived query methods.
 
**15. Write Comprehensive Tests:**
  - Include unit tests for business logic, integration tests for services and repositories, and API tests for controllers. Use tools like JUnit 5, Mockito, and Testcontainers.
 
**16. Document Your API with OpenAPI/Swagger:**
  - Integrate SpringDoc OpenAPI to automatically generate API documentation.
 
**17. Use Actuator for Monitoring:**
  - Enable Spring Boot Actuator endpoints (`/health`, `/info`, `/metrics`) for monitoring and management of your application.
 
**18. Avoid Anemic Domain Models:**
  - Don't just make entities plain data holders. Add behavior to your domain objects where it makes sense, following Domain-Driven Design principles.
 
**19. Proper Error Response Structure:**
  - Return consistent and informative error responses (e.g., JSON with `status`, `message`, `timestamp`). Avoid exposing internal details.
 
**20. Use a Multi-Module Project for Larger Applications:**
  - For complex applications, consider structuring your project into multiple modules (e.g., `api`, `service`, `domain`, `persistence`) to promote modularity and clear separation of concerns.
...
# This slash command template helps generate a Spring Boot REST API based on the defined spec.
# 
# Usage:
#   /generate-spring-boot-api <description_of_api>
#   e.g., /generate-spring-boot-api "Create a user management service with CRUD operations for User entity."
#
# Provide details about the API you want to build, and Copilot will assist you by referencing
# the "spring.boot.rest.api.md" specification file.

# You can also specify entities or specific requirements:
#   /generate-spring-boot-api "Implement a product catalog API with Product entity, supporting search and filtering."
#   /generate-spring-boot-api "Build an order processing service, including Order and OrderItem entities, with transactional behavior."
#
# The agent will ensure the generated code adheres to the defined:
# - Technology Stack (Java 21, Spring Boot 3.3+, Maven, PostgreSQL, OpenAPI, etc.)
# - Project Structure (config, controller, dto, model, repository, service, exception)
# - Development Workflow (DTOs, Controller, Service, Repository, Exception Handling, Caching)
# - Coding Standards (Constructor injection, DTO usage, Validation, Global Exception Handling, Logging, Transactions)
# - Testing Best Practices (Unit, Integration, Coverage)
# - Containerization (Dockerfile structure)

# Describe your desired Spring Boot REST API:

...
- **Lombok Usage**:
    - DTOs and custom error responses utilize `@Getter`, `@Setter`, `@NoArgsConstructor`, and `@AllArgsConstructor`.
    - JPA Entities must explicitly use `@Getter`, `@Setter`, `@NoArgsConstructor`, and `@AllArgsConstructor`. **Do not use `@Data` on entities.**
- **API Versioning**: All REST API endpoints must be prefixed with `/api/v1` (e.g., `@RequestMapping("/api/v1/resource")`).
- **DTOs for API Interactions**: Exclusively use Data Transfer Objects (DTOs) for all API requests and responses. JPA entities should never be directly exposed at the API layer.
- **Input Validation**: Apply Jakarta Bean Validation annotations (`@NotNull`, `@NotBlank`, `@Size`, `@Pattern`, etc.) comprehensively on all incoming DTOs to ensure data integrity and validation at the API boundary.
- **Dependency Injection**: Favor constructor injection for all dependencies in services and controllers to ensure immutability and testability.
- **Transactional Management**: Apply `@Transactional` to service methods that modify data. For read-only operations, use `@Transactional(readOnly = true)` to optimize database interactions.
- **Caching**: Implement caching mechanisms using `@Cacheable`, `@CachePut`, and `@CacheEvict` annotations on service methods for performance-critical operations. Ensure cache configuration is externalized in dedicated `CacheConfig` classes.
- **Global Exception Handling**: Centralize exception handling using a `@RestControllerAdvice` class with `@ExceptionHandler` methods to catch custom exceptions (e.g., `ResourceNotFoundException`) and standard Spring exceptions (e.g., `MethodArgumentNotValidException`). Return consistent `ErrorResponse` DTOs with appropriate HTTP status codes.
- **JPA Entity Best Practices**: Define JPA entities using `@Entity`, `@Table`, `@Id`, `@GeneratedValue`, and specific `@Column` annotations for precise database mapping.
- **Dedicated Configuration Classes**: Organize application configurations (e.g., Cache, OpenAPI, Security) into separate `@Configuration` classes within the `config` package.
